...
class PacketTypes(Enum):
    CONNECTION_NONE = 0
    CONNECTION_REQ = 1  # request
    CONNECTION_RSP = 2  # response
    CLIENT_LIST_REQ = 3
    CLIENT_LIST_RSP = 4
    STOCK_LIST_REQ = 5
    STOCK_LIST_RSP = 6
...
class Packet:
    def __init__(self): ### three member 
        self.m_type = 0
        self.m_msg_size = 0
        self.m_data_size = 0
        self.m_data = ""
    ...
    def deserialize(self, message):
        msg_len = len(message)
        msg_unpack_string = '<iii' + str(msg_len - 12) + 's' ### first three as int and following the string
